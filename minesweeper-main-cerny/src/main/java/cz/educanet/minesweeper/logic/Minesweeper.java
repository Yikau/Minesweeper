...
public class Minesweeper {
    ...
    public int getAdjacentBombCount(int x, int y) {
        ...
        if(topleft && grid[x - 1][y - 1].getBomb()) bombpoc++; // top left
        if(topright && grid[x + 1][y - 1].getBomb())bombpoc++; // top right
        if(botright && grid[x + 1][y + 1].getBomb()) bombpoc++; // bot right
        if(botleft && grid[x - 1][y + 1].getBomb()) bombpoc++; // bot left

        if((topleft || topright) && grid[x][y - 1].getBomb()) bombpoc++; // top
        if((botright || botleft) && grid[x][y + 1].getBomb()) bombpoc++; // bot
        if((botleft || topleft) && grid[x - 1][y].getBomb()) bombpoc++; // left
        if((topright || botright) && grid[x + 1][y].getBomb()) bombpoc++; // right
